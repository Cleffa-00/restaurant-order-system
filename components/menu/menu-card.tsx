...
export function MenuCard({ item, onClick, onCartAnimation }: MenuCardProps) {
  ...
  const triggerFlyingAnimation = (isDecrease = false) => {
    ...
    ghost.style.transition = isDecrease
      ? "all 0.6s cubic-bezier(0.55, 0.085, 0.68, 0.53)" // Faster, more direct for decrease
      : "all 0.8s cubic-bezier(0.25, 0.46, 0.45, 0.94)" // Smooth for increase
    ...
    requestAnimationFrame(() => {
      if (isDecrease) {
        // For decrease: reverse animation (cart to image position, then fade)
        ghost.style.transform = `translate(${cartCenterX - imageCenterX}px, ${cartCenterY - imageCenterY}px) scale(0.1)`
        ghost.style.opacity = "0"
      } else {
        // For increase: normal animation (image to cart)
        ghost.style.transform = `translate(${cartCenterX - imageCenterX}px, ${cartCenterY - imageCenterY}px) scale(0.1)`
        ghost.style.opacity = "0"
      }
    })
  ...
  return (
    <Card
      ...
      className={cn(
        ...
        item.available && (hasOptions || totalQuantity === 0) && "cursor-pointer",
      )}
  ...
  <div>
    ...
    {hasOptions && (
      <div className="mt-2 text-xs text-gray-500 flex items-center">
        ...
        {item.optionGroups.length} customization option{item.optionGroups.length !== 1 ? "s" : ""}
      </div>
    )}
  </div>
      ...
      (totalQuantity === 0 ? (
        hasOptions ? (
          ...
          <Button
            onClick={handleAddToCart}
            className={cn(
              "bg-gray-900 text-white font-medium w-7 h-7 sm:w-8 sm:h-8 md:w-9 md:h-9 text-lg sm:text-xl md:text-2xl font-bold rounded-full flex items-center justify-center p-0 leading-none",
              "hover:bg-gray-800 active:bg-gray-950 transition-all duration-200",
              "shadow-sm hover:shadow-md focus:ring-2 focus:ring-gray-900/20 focus:ring-offset-2",
              "pb-[1px]"
            )}
