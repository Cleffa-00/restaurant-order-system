...
export class OrdersAPI {
  ...
  static async updateOrderStatus(orderId: string, data: UpdateOrderStatusRequest) {
    return apiClient.patch<Order>(`/api/admin/orders/${orderId}/status`, data)
  }
  ...
  static async updatePaymentStatus(orderId: string, data: UpdatePaymentStatusRequest) {
    return apiClient.patch<Order>(`/api/admin/orders/${orderId}/payment`, data)
  }
  ...
  static async deleteOrder(orderId: string) {
    return apiClient.delete(`/api/admin/orders/${orderId}`)
  }
  ...
  static async getOrderStats(date?: string) {
    const endpoint = date 
      ? `/api/admin/orders/stats?date=${date}`
  ...
  static async batchUpdateStatus(orderIds: string[], status: UpdateOrderStatusRequest['status']) {
    return apiClient.patch('/api/admin/orders/batch/status', {
      orderIds,
      status
    })
  }
  ...
  static async searchOrders(query: string) {
    return apiClient.get<Order[]>(`/api/admin/orders/search?q=${encodeURIComponent(query)}`)
  }
}
