// lib/api/admin/option-groups.ts
...
export async function getOptionGroups(menuItemId?: string): Promise<AdminOptionGroup[]> {
  try {
    const url = menuItemId ? `/api/admin/option-groups?menuItemId=${menuItemId}` : '/api/admin/option-groups'
...
export async function getOptionGroup(id: string): Promise<AdminOptionGroup> {
  try {
    const result = await apiClient.get<AdminOptionGroup>(`/api/admin/option-groups/${id}`)
...
export async function createOptionGroup(data: CreateOptionGroupRequest & { menuItemId: string }): Promise<AdminOptionGroup> {
  try {
    const result = await apiClient.post<AdminOptionGroup>('/api/admin/option-groups', data)
    if (!result.data) throw new Error('No data returned from server')
    return result.data
  } catch (error) {
...
export async function updateOptionGroup(id: string, data: UpdateOptionGroupRequest): Promise<AdminOptionGroup> {
  try {
    const result = await apiClient.put<AdminOptionGroup>(`/api/admin/option-groups/${id}`, data)
    if (!result.data) throw new Error('No data returned from server')
    return result.data
  } catch (error) {
...
export async function deleteOptionGroup(id: string): Promise<void> {
  try {
    await apiClient.delete(`/api/admin/option-groups/${id}`)
  } catch (error) {
