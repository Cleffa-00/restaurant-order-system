...
export async function getCategories(): Promise<AdminCategory[]> {
  try {
    const result = await apiClient.get<AdminCategory[]>('/api/admin/categories')
    return result.data || []
  } catch (error) {
...
export async function createCategory(data: CreateCategoryRequest): Promise<AdminCategory> {
  try {
    const result = await apiClient.post<AdminCategory>('/api/admin/categories', data)
...
export async function updateCategory(id: string, data: UpdateCategoryRequest): Promise<AdminCategory> {
  try {
    const result = await apiClient.put<AdminCategory>(`/api/admin/categories/${id}`, data)
    if (!result.data) throw new Error('No data returned from server')
    return result.data
  } catch (error) {
...
export async function reorderCategories(categoryIds: string[]): Promise<void> {
  try {
    await apiClient.post('/api/admin/categories/reorder', { categoryIds })
  } catch (error) {
